...
impl CommunityLedger {
    ...
    pub fn record_contribution(
        ...
        env.storage()
            .persistent()
            .set(&DataKey::Contribution(task_id), &log_entry);
        ...
        prov.community_score =
            Self::calc_score(prov.hours_given, prov.hours_taken, prov.tasks_given);
        prov.standing = Self::score_to_standing(prov.community_score);

        if prov.standing != old_standing {
            env.events()
                .publish((EV_STANDING_UP, provider.clone()), (prov.community_score,));
        }
        env.storage()
            .persistent()
            .set(&DataKey::Member(provider.clone()), &prov);

        // Update total hours counter
        let total: u64 = env.storage().instance().get(&TOTAL_HOURS).unwrap_or(0u64);
        env.storage()
            .instance()
            .set(&TOTAL_HOURS, &(total + hours as u64));

        env.events()
            .publish((EV_CONTRIBUTED, provider.clone()), (task_id, hours));

        log!(
            &env,
            "Contribution recorded: task={} provider={} hours={}",
            task_id,
            provider,
            hours
        );
    ...
    pub fn record_consumption(env: Env, requester: Address, hours: u32) {
        ...
        if profile.standing != old_standing {
            env.events().publish(
                (EV_STANDING_UP, requester.clone()),
        ...
        env.storage()
            .persistent()
            .set(&DataKey::Member(requester.clone()), &profile);
    }
